...
public class RussianRoulette2 { // 강사님이랑 작성한 것
	...
	public static void main(String[] args) {
		...
		// 횟수가 정확하면 for문 아니면 while문 이유 : for문은 반복문이 끝나면 무조건 증감 while문은 증감을 내가 조건으로 조절할 수 있음.
		while(checkNum < bullet) {
			int position = (int) (Math.random()*bulletPos.length);
			if(bulletPos[position]) {
				continue; // while 문에서 continue를 그냥 진행하면 제어변수에 변동이 없음. for문은 남은 코드를 진행하고 자동으로 증감.
			} else {
...
//		System.out.println(Arrays.toString(bulletPos));
		...
		while(true) {
			...
			if(bulletPos[realBulletPos]) { // 총알 격발!
				...
				for(int i=startIdx; i<players.length-1; i++) { // 맨 끝의 값은 땡겨올 값이 없으므로 length-1
					players[i] = players[i+1];
				}
				
				String[] temp = new String[players.length-1]; // [playerNum]을 해도 무방 위에서 값을 하나 뺐기 때문에
