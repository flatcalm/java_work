...
/*
 ...
 - 메서드의 호출은 곧 메서드를 사용하는 것을 의미하며,
 	1. 메서드가 필요로 하는 데이터(매개값)을 호출부에서 전달하면 
...
public class MethodExample1 {
	...
	static int calcTotal(int x) {	// static [리턴하는 값의 타입] [메서드 이름](매개 변수) 
									// { [실행할 값] return [리턴할 값]; }
									// 메서드 이름 = 메서드의 이름 main 함수에서 호출할 때 사용될 이름
									// 매개 변수 = 메서드에서 사용될 변수값(main에서 받을 값)
		int total = 0;
		for(int i=1; i<=x; i++) {
			total += i;
		}
